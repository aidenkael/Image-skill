...
- 源组件与目标区域都以 `bbox_list` 显式选中：Image 1 的框选中精确源组件（周边裁剪只是上下文），
  Image 2 的框选中唯一可编辑目标区域；
...
3. **base 阶段**：`python crystal/crystal.py base --input 原图 --analysis analysis.json --output base.png --workdir work`
   （内部：新鲜 analysis 校验 → 清理裁剪 → 每组带上下文的代表资产（source_bbox 选中精确组件）留存 workdir
...
6. **compose 阶段**：`python crystal/crystal.py compose --input base.png --source 原图 --analysis analysis.json --placements placements.json --output candidate.png --workdir work`
   ...
   每次 Image 1 = 代表资产（source_bbox 选源组件）、Image 2 = 同一张干净 base（target bbox 选目标区域）
...
- 合并区域 = 目标框外扩 45%（覆盖接触阴影/反射），边缘高斯羽化；区域外像素与 base 完全一致。
...
- insert：只编辑 Image 2 的选中目标区域；恰好一件、保真源组件（几何/比例/颜色/透明度/内含物/纹理/表面）、
  匹配 base 的光照与表面、自然接触阴影与局部反射；目标区域外必须不变；
  不复制/模仿 Image 2 的其他物体、不加第二件、不改手镯；无悬浮、无抠图边、无贴纸感；
